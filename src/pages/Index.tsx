...
function InvitePage({ fromNick, onJoin }: { fromNick: string; onJoin: () => void }) {
  return (
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-4 py-16 relative z-10">
        <div className="w-full max-w-lg text-center animate-fade-in">
          ...
          <div className="inline-flex items-center gap-2 rounded-full px-4 py-1.5 bg-gold/10 border border-gold/25 mb-6">
            ...
            <span className="font-mono text-sm text-gold font-medium">{fromNick}</span>
          </div>

          <h1 className="font-display text-5xl sm:text-6xl gold-text leading-tight mb-4">Войдите в Prime</h1>
        ...
        <div className="w-full max-w-lg mt-16 grid grid-cols-2 gap-4">
          {FEATURES.map((f, i) => (
            <div key={f.title} className="rounded-2xl bg-card/60 gold-border p-5 text-left animate-fade-in" style={{ animationDelay: `${200 + i * 80}ms` }}>
...
export default function Index() {
  ...
  const [showInvitePage, setShowInvitePage] = useState(!!inviteNick);
  ...
  if (showInvitePage && inviteNick && !user) {
    return <InvitePage fromNick={inviteNick} onJoin={() => setShowInvitePage(false)} />;
  }
